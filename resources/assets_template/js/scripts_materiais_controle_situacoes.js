...
function patrimonioSituacoesTable(material_entrada_item_id) {
    var url_atual = window.location.protocol+'//'+window.location.host+'/';

    // Acessar rota
    fetch(url_atual+'patrimonio/patrimonio_situacoes/'+material_entrada_item_id, {
        method: 'GET',
        ...
        if (patrimonio_situacoes.length > 0) {
            ...
            patrimonio_situacoes.forEach(dado => {
                ...
                grade += '  <td>'+dado.anteriorMaterialSituacaoName+'</td>';
                grade += '  <td>'+dado.atualMaterialSituacaoName+'</td>';
                grade += '</tr>';
            });
