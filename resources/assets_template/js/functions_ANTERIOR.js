...
async function cartaoEmergencialGerarPDF(op = 0, ids = 0, gerar = 2, traducao = 'pt', divId = '') {
    ...
    try {
        ...
        for (const pessoa of pessoas) {
            ...
            if (gerar === 1) {
                if (traducao == 'pt') {
                    var qrCodePngCaminho = 'build/assets/qrcodes/funcionarios/qrcode_cartao_emergencial_pt_' + pessoa.id + '.png';
                }
                if (traducao == 'en') {
                    var qrCodePngCaminho = 'build/assets/qrcodes/funcionarios/qrcode_cartao_emergencial_en_'+pessoa.id+'.png';
                }
...
async function cartaoEmergencialDesenhar(doc, x, y, pessoa, qrCodePngCaminho, traducao) {
    ...
    doc.addImage(qrCodePngCaminho, "PNG", x + largura - 23, y + 29, 20, 20); // QR à direita inferior
