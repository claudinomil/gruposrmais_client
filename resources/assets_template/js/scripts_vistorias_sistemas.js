async function executarVistoriaSistema(edificacao_id) {
    try {
        const response = await fetch('vistorias_sistemas', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ edificacao_id: edificacao_id })
        });

        const data = await response.json();

        if (data.success) {
            return data.vistoria_sistema_id;
            ...
            alertSwal('warning', 'Vistoria Sistemas', data.error, 'true', 20000);
        } else {
            alert('Erro interno');
        }
    } catch (error) {
        alert('Erro Vistoria Sistema: ' + error);
    }
}

let vistoriaSistemaDadoId = 0; //Id da Pergunta que está sendo manipulada na tabela vistorias_sistemas_dados

function validar_frm_vistorias_sistemas() {
    var validacao_ok = true;
    var mensagem = '';

    //Se diferente de inclusão validar campos vistoria_sistema_status_id, datas e horas''''''''''''''''''''''''''''''''''''
    if (document.getElementById('frm_operacao').value != 'create') {
        //Campo: vistoria_sistema_status_id
        if (validacao({op: 1, value: document.getElementById('vistoria_sistema_status_id').value}) === false) {
            validacao_ok = false;
            mensagem += 'Status requerido.' + '<br>';
        } else {
            //Campo: vistoria_sistema_status_id (deve ser um número)
            if (validacao({op: 4, value: document.getElementById('vistoria_sistema_status_id').value}) === false) {
                validacao_ok = false;
                mensagem += 'Status deve ser escolhido.' + '<br>';
            } else {
                var status_id = document.getElementById('vistoria_sistema_status_id').value;
    ...
    //Campo: edificacao_id (requerido)
    if (validacao({op:1, value:document.getElementById('edificacao_id').value}) === false) {
        validacao_ok = false;
        mensagem += 'Edificação requerido.' + '<br>';
    } else {
        //Campo: edificacao_id (deve ser um número)
        if (validacao({op:4, value: document.getElementById('edificacao_id').value}) === false) {
            validacao_ok = false;
            mensagem += 'Edificação deve ser escolhido.' + '<br>';
        }
    }

    //Campo: responsavel_funcionario_id (requerido)
    if (validacao({op:1, value:document.getElementById('responsavel_funcionario_id').value}) === false) {
        validacao_ok = false;
        mensagem += 'Responsável requerido.' + '<br>';
    } else {
        //Campo: responsavel_funcionario_id (deve ser um número)
        if (validacao({op:4, value: document.getElementById('responsavel_funcionario_id').value}) === false) {
            validacao_ok = false;
 ...
 */
function formControle(op) {
    let informacoesGerais = document.getElementById('divInformacoesGerais');
    let cliente = document.getElementById('divCliente');
    let edificacao = document.getElementById('divEdificacao');
    let responsavel = document.getElementById('divResponsavel');
    let finalizar = document.getElementById('divFinalizar');
    let perguntas = document.getElementById('divMapaPreventivo');

    informacoesGerais.style.display = 'none';
    cliente.style.display = 'none';
    edificacao.style.display = 'none';
...
function abrirModalObservacao(vistoria_sistema_dado_id) {
    event.preventDefault();

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    document.getElementById('modal_observacao_texto').value = document.getElementById('observacao_texto_'+vistoriaSistemaDadoId).value;
...
function abrirModalFotografia(vistoria_sistema_dado_id) {
    event.preventDefault();

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    document.getElementById('modalFotografiaFile').value = '';

    new bootstrap.Modal(document.getElementById('modalFotografia')).show();

    //Posicionar elemento no topo
    posicionarPergunta();
}

function abrirModalPdf(vistoria_sistema_dado_id) {
    event.preventDefault();
...
function observacaoEnviar() {
    if (document.getElementById('modal_observacao_texto').value == '') {
        if (!confirm('Tem certeza que deseja enviar uma Observação vazia?')) return;
    }

    document.getElementById('divObservacaoTexto_'+vistoriaSistemaDadoId).innerHTML = document.getElementById('modal_observacao_texto').value;
    document.getElementById('observacao_texto_'+vistoriaSistemaDadoId).value = document.getElementById('modal_observacao_texto').value;

    //Show classObservacaoFotografiaPdf
    observacaoFotografiaPdfShow();

    //Fechar Modal
    bootstrap.Modal.getInstance(document.getElementById('modalObservacao')).hide();

    //Salvar
    salvarDadosPergunta(vistoriaSistemaDadoId);
}

function fotografiaEnviar(fileInput) {
    ...
    function uploadFoto(blob) {
        ...
        const container = document.getElementById(`divFotografiaFotos_${vistoriaSistemaDadoId}`);
        ...
        fetch('vistorias_sistemas/pergunta/uploadFotografia/' + vistoriaSistemaDadoId + '/' + slotLivre, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                ...
                fileInput.value = '';
                bootstrap.Modal.getInstance(document.getElementById('modalFotografia')).hide();
            })
            .catch(() => alert('Erro na comunicação com o servidor.'));
    }
}

function pdfEnviar(fileInput) {
    const files = fileInput.files;
    ...
    function uploadPdf(pdfFile) {
        // Encontra o próximo slot livre
        const container = document.getElementById(`divPdf_${vistoriaSistemaDadoId}`);
        ...
        fetch('vistorias_sistemas/pergunta/uploadPdf/' + vistoriaSistemaDadoId + '/' + slotLivre, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            },
...
function fotografiaInserir(imagemUrl) {
    const container = document.getElementById(`divFotografiaFotos_${vistoriaSistemaDadoId}`);
    if (!container) return;

    for (let i = 1; i <= 3; i++) {
        const img = document.getElementById(`img_${vistoriaSistemaDadoId}_${i}`);
        const inputHidden = document.getElementById(`fotografia_${vistoriaSistemaDadoId}_${i}`);
...
function pdfInserir(pdfUrl) {
    ...
    for (let i = 1; i <= 3; i++) {
        const embed = document.getElementById(`img_pdf_${vistoriaSistemaDadoId}_${i}`);
        const inputHidden = document.getElementById(`pdf_${vistoriaSistemaDadoId}_${i}`);
...
function removerFotografia(button) {
    const vistoria_sistema_dado_id = button.dataset.vistoria_sistema_dado_id;
    const slot = button.dataset.slot;

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    fetch(`vistorias_sistemas/pergunta/removerFotografia/${vistoria_sistema_dado_id}/${slot}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        .then(data => {
            if (data.success) {
                const img = document.getElementById(`img_${vistoria_sistema_dado_id}_${slot}`);
                const input = document.getElementById(`fotografia_${vistoria_sistema_dado_id}_${slot}`);
...
function removerPdf(button) {
    const vistoria_sistema_dado_id = button.dataset.vistoria_sistema_dado_id;
    const slot = button.dataset.slot;

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    fetch(`vistorias_sistemas/pergunta/removerPdf/${vistoria_sistema_dado_id}/${slot}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        .then(data => {
            if (data.success) {
                const embed = document.getElementById(`img_pdf_${vistoria_sistema_dado_id}_${slot}`);
                const input = document.getElementById(`pdf_${vistoria_sistema_dado_id}_${slot}`);
...
function visualizarFotografia(button) {
    const vistoria_sistema_dado_id = button.dataset.vistoria_sistema_dado_id;
    const slot = button.dataset.slot;
...
function visualizarPdf(button) {
    const vistoria_sistema_dado_id = button.dataset.vistoria_sistema_dado_id;
    const slot = button.dataset.slot;

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    const embed = document.getElementById(`pdf_${vistoria_sistema_dado_id}_${slot}`);
...
function salvarDadosPergunta(vistoria_sistema_dado_id) {
    //Mostrar divBloqueio para bloquear interações
    document.getElementById('divBloqueio').style.display = 'block';

    vistoriaSistemaDadoId = vistoria_sistema_dado_id;

    //Verificar Hiddens
    var vistoria_sistema_id = document.getElementById('vistoria_sistema_id_'+vistoriaSistemaDadoId).value;
    var titulo = document.getElementById('titulo_'+vistoriaSistemaDadoId).value;
    var subtitulo = document.getElementById('subtitulo_'+vistoriaSistemaDadoId).value;
    var pergunta = document.getElementById('pergunta_'+vistoriaSistemaDadoId).value;

    //Verificar Resposta
    var resposta = 0;

    if (document.getElementById('resposta_'+vistoriaSistemaDadoId+'_1').checked) {resposta = 1;}
    if (document.getElementById('resposta_'+vistoriaSistemaDadoId+'_2').checked) {resposta = 2;}
    if (document.getElementById('resposta_'+vistoriaSistemaDadoId+'_3').checked) {resposta = 3;}

    //Verificar Quantidade
    var quantidade = document.getElementById('quantidade_'+vistoriaSistemaDadoId).value;

    //Verificar Observação
    var observacao = document.getElementById('observacao_texto_'+vistoriaSistemaDadoId).value;

    //Verificar Fotografias
    var fotografia_1 = document.getElementById('fotografia_'+vistoriaSistemaDadoId+'_1').value;
    var fotografia_2 = document.getElementById('fotografia_'+vistoriaSistemaDadoId+'_2').value;
    var fotografia_3 = document.getElementById('fotografia_'+vistoriaSistemaDadoId+'_3').value;

    //Verificar PDFs
    var pdf_1 = document.getElementById('pdf_'+vistoriaSistemaDadoId+'_1').value;
    var pdf_2 = document.getElementById('pdf_'+vistoriaSistemaDadoId+'_2').value;
    var pdf_3 = document.getElementById('pdf_'+vistoriaSistemaDadoId+'_3').value;

    //Montar dados e Salvar na tabela'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    let payload = {vistoria_sistema_id, titulo, subtitulo, pergunta, resposta, quantidade, observacao, fotografia_1, fotografia_2, fotografia_3, pdf_1, pdf_2, pdf_3};

    //Acessar rota
    fetch('vistorias_sistemas/pergunta/updatePergunta/'+vistoriaSistemaDadoId, {
        method: 'POST',
        headers: {
            'REQUEST-ORIGIN': 'fetch',
            'Content-Type': 'application/json',
        ...
        alert('Erro salvarDadosPergunta: '+error);
    }).finally(() => {
        //Oculta divBloqueio independentemente do resultado
...
function posicionarPergunta() {
    //Posicionar elemento no topo
    const elemento = document.getElementById('divPergunta_' + vistoriaSistemaDadoId);
...
function observacaoFotografiaPdfShow() {
    ...
    observacaoFotografiasPdfs.forEach(element => {
        const vistoria_sistema_dado_id = element.getAttribute('data-vistoria_sistema_dado_id');

        var observacao_texto = document.getElementById('observacao_texto_'+vistoria_sistema_dado_id).value;
        var fotografia_1 = document.getElementById('fotografia_'+vistoria_sistema_dado_id+'_1').value;
        var fotografia_2 = document.getElementById('fotografia_'+vistoria_sistema_dado_id+'_2').value;
        var fotografia_3 = document.getElementById('fotografia_'+vistoria_sistema_dado_id+'_3').value;
        var pdf_1 = document.getElementById('pdf_'+vistoria_sistema_dado_id+'_1').value;
        var pdf_2 = document.getElementById('pdf_'+vistoria_sistema_dado_id+'_2').value;
        var pdf_3 = document.getElementById('pdf_'+vistoria_sistema_dado_id+'_3').value;

        if (observacao_texto != '' || fotografia_1 != '' || fotografia_2 != '' || fotografia_3 != '' || pdf_1 != '' || pdf_2 != '' || pdf_3 != '') {
            document.getElementById('divObservacaoFotografiaPdf_' + vistoria_sistema_dado_id).style.display = '';
        } else {
            document.getElementById('divObservacaoFotografiaPdf_'+vistoria_sistema_dado_id).style.display = 'none';
        }
    });
}

function gerarHtmlPerguntas(vistorias_sistemas_dados) {
    ...
    vistorias_sistemas_dados.forEach(dado => {
        opcoes = dado.opcoes;
        ...
        //Verificar se deixa aberto a divObservacaoFotografiaPdf_''''
        var divObservacaoFotografiaPdf_display = '';

        if (campo_observacao == '' && campo_fotografia_1 == '' && campo_fotografia_2 == '' && campo_fotografia_3 == '' && campo_pdf_1 == '' && campo_pdf_2 == '' && campo_pdf_3 == '') {
            var divObservacaoFotografiaPdf_display = 'none';
        }
        //''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

        //Html
        html += `<div class="row pb-3">
            <input type="hidden" name="vistoria_sistema_id_${dado.id}" id="vistoria_sistema_id_${dado.id}" value="${dado.vistoria_sistema_id}">
            <input type="hidden" name="titulo_${dado.id}" id="titulo_${dado.id}" value="${dado.titulo}">
            <input type="hidden" name="subtitulo_${dado.id}" id="subtitulo_${dado.id}" value="${dado.subtitulo}">
            <input type="hidden" name="pergunta_${dado.id}" id="pergunta_${dado.id}" value="${dado.pergunta}">`;
        ...
        if (dado.subtitulo) {
            ...
            html += `<div class="row col-12 bg-light ms-3 py-2" id="divPergunta_${dado.id}">
                        <div class="col-12 col-md-6">${contadorTitulo}.${String.fromCharCode(64 + contadorSubtitulo)}.${contadorPergunta}. ${dado.pergunta}</div>`;
        } else {
            contadorPergunta++;

            html += `<div class="row col-12 bg-light ms-3 py-2 px-0" id="divPergunta_${dado.id}">
        ...
        if ([4, 8, 11, 13, 15, 17, 19, 21, 22, 24, 25, 26, 28, 29, 30, 31].includes(opcoes)) {
            html += `<div class="form-check-success text-start flex-grow-1 px-1">
                        <input class="form-check-input" type="radio" onclick="salvarDadosPergunta(${dado.id})" name="resposta_${dado.id}" id="resposta_${dado.id}_1" ${campo_resposta_1} ${campo_resposta_disabled}>
                        <label class="form-check-label" for="resposta_${dado.id}_1">Sim</label>
                    </div>
                    <div class="form-check-success text-start flex-grow-1 px-1">
                        <input class="form-check-input" type="radio" onclick="salvarDadosPergunta(${dado.id})" name="resposta_${dado.id}" id="resposta_${dado.id}_2" ${campo_resposta_2} ${campo_resposta_disabled}>
                        <label class="form-check-label" for="resposta_${dado.id}_2">Não</label>
                    </div>
                    <div class="form-check-success text-start flex-grow-1 px-1">
                        <input class="form-check-input" type="radio" onclick="salvarDadosPergunta(${dado.id})" name="resposta_${dado.id}" id="resposta_${dado.id}_3" ${campo_resposta_3} ${campo_resposta_disabled}>
                        <label class="form-check-label" for="resposta_${dado.id}_3">NI</label>
                    </div>`;
        }
        ...
        if ([5, 9, 12, 14, 15, 18, 20, 21, 23, 24, 25, 27, 28, 29, 30, 31].includes(opcoes)) {
            html += `<div class="form-group text-center flex-grow-1 px-1">
                        ...
                        <input type="text" class="form-control form-control-sm d-inline-block text-center" onchange="salvarDadosPergunta(${dado.id})" name="quantidade_${dado.id}" id="quantidade_${dado.id}" value="${dado.quantidade}" placeholder="Qtd" min="0" max="999" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 3)" style="width: 40px;" ${campo_quantidade_disabled}>
                    </div>`;
        }
        ...
        if (operacao != 'view') {
            ...
            if ([1, 6, 7, 8, 9, 16, 17, 18, 19, 20, 21, 26, 27, 28, 29, 31].includes(opcoes)) {
                html += `<div class="text-end flex-grow-1 px-1">
                            <a href="#" onclick="abrirModalObservacao(${dado.id})" title="Anexar Observação" id="btnObservacao_${dado.id}">
            ...
            if ([3, 7, 10, 13, 14, 16, 19, 20, 22, 23, 25, 26, 27, 29, 30, 31].includes(opcoes)) {
                html += `<div class="text-end flex-grow-1 px-1">
                            <a href="#" onclick="abrirModalFotografia(${dado.id})" title="Anexar Fotografia" id="btnFotografia_${dado.id}">
            ...
            if ([2, 6, 10, 11, 12, 16, 17, 18, 22, 23, 24, 26, 27, 28, 30, 31].includes(opcoes)) {
                html += `<div class="text-end flex-grow-1 px-1">
                            <a href="#" onclick="abrirModalPdf(${dado.id})" title="Anexar PDF" id="btnPdf_${dado.id}">
        ...
        html += `
            ...
            <div class="row classObservacaoFotografiaPdf" id="divObservacaoFotografiaPdf_${dado.id}" data-vistoria_sistema_dado_id="${dado.id}" style="display: ${divObservacaoFotografiaPdf_display};">
                <hr class="ms-3 my-2" />`;

        //Observação
        if ([1, 6, 7, 8, 9, 16, 17, 18, 19, 20, 21, 26, 27, 28, 29, 31].includes(opcoes)) {
            html += `<div class="col-12 col-md-4 text-success" id="divObservacao_${dado.id}">
                    Observação<br>
                    <div class="col-12 text-black justify-content-start" id="divObservacaoTexto_${dado.id}"> ${campo_observacao}</div>
                    <textarea id="observacao_texto_${dado.id}" name="observacao_texto_${dado.id}" style="display: none;">${campo_observacao}</textarea>
                </div>`;
        }

        //Imagens
        if ([3, 7, 10, 13, 14, 16, 19, 20, 22, 23, 25, 26, 27, 29, 30, 31].includes(opcoes)) {
            html += `<div class="col-12 col-md-4 text-primary" id="divFotografia_${dado.id}">
                    Fotografia(s)<br>
                    <div class="col-12 d-flex flex-wrap text-black" id="divFotografiaFotos_${dado.id}">`;
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                html += `
                    <div class="m-2 d-inline-block text-center position-relative container-foto" style="${display_fotografia}">
                        <button type="button" class="btn btn-sm btn-success position-absolute top-0 start-0 p-0 px-1 visualizar-foto" style="${display_fotografia}" data-vistoria_sistema_dado_id="${dado.id}" data-slot="${i}"><i class="fas fa-search"></i></button>
                        <img src="${campo_fotografia}" class="img-thumbnail img-foto" style="width: 70px; height: 70px; object-fit: cover; ${display_fotografia}" name="img_${dado.id}_${i}" id="img_${dado.id}_${i}">
                        <input type="hidden" name="fotografia_${dado.id}_${i}" id="fotografia_${dado.id}_${i}" value="${campo_fotografia}">
                `;

                if (operacao != 'view') {
                    html += `
                        <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 p-0 px-1 remover-foto" style="${display_fotografia}" data-vistoria_sistema_dado_id="${dado.id}" data-slot="${i}"><i class="fas fa-times"></i></button>
                    `;
                }
        ...
        //Pdf
        if ([2, 6, 10, 11, 12, 16, 17, 18, 22, 23, 24, 26, 27, 28, 30, 31].includes(opcoes)) {
            html += `<div class="col-12 col-md-4 text-danger" id="divPdf_${dado.id}">
                    PDF(s)<br>
                    <div class="col-12 d-flex flex-wrap text-black" id="divPdfPdfs_${dado.id}">`;
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                html += `
                    <div class="m-2 d-inline-block text-center position-relative container-pdf" style="${display_pdf}">
                        <button type="button" class="btn btn-sm btn-success position-absolute top-0 start-0 p-0 px-1 visualizar-pdf" style="${display_pdf}" data-vistoria_sistema_dado_id="${dado.id}" data-slot="${i}"><i class="fas fa-search"></i></button>
                        <img src="${url_atual+'build/assets/images/image_pdf.png'}" class="img-thumbnail img-pdf" style="width: 70px; height: 70px; object-fit: cover; ${display_pdf}" name="img_pdf_${dado.id}_${i}" id="img_pdf_${dado.id}_${i}">
                        <input type="hidden" name="pdf_${dado.id}_${i}" id="pdf_${dado.id}_${i}" value="${campo_pdf}">
                `;

                if (operacao != 'view') {
                    html += `
                        <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 p-0 px-1 remover-pdf" style="${display_pdf}" data-vistoria_sistema_dado_id="${dado.id}" data-slot="${i}"><i class="fas fa-times"></i></button>
                    `;
                }
...
async function vistoriaSistemaGerarPdf(vistoria_sistema_id, traducao='pt') {
    try {
        ...
        await gerarPDF(vistoria_sistema_id, traducao);
    } catch (e) {
...
async function gerarPDF(vistoria_sistema_id, traducao='pt') {
    //Acessar rota vistorias_sistemas
    let response = await fetch('vistorias_sistemas/' + vistoria_sistema_id, {
        method: 'GET',
    ...
    if (data.success) {
        ...
        const edificacao_id = data.success.edificacao_id; // edificacao_id para ser usado nas funções internas
        var pageTopo = 'build/assets/images/vistoria_sistema_topo.png';
        var pageRodape = 'build/assets/images/vistoria_sistema_rodape.png';

        //Verificar se existe topo para cliente''''''''''''''''''''''''''''''''''''''''''''''''''''''
        var topoClienteExiste = await arquivoExiste('build/assets/images/vistoria_sistema_topo_cliente_'+cliente_id+'.png');
        if (topoClienteExiste === true) {pageTopo = 'build/assets/images/vistoria_sistema_topo_cliente_'+cliente_id+'.png';}
        //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

        //Verificar se existe rodapé para cliente''''''''''''''''''''''''''''''''''''''''''''''''''''
        var rodapeClienteExiste = await arquivoExiste('build/assets/images/vistoria_sistema_rodape_cliente_'+cliente_id+'.png');
        if (rodapeClienteExiste === true) {pageRodape = 'build/assets/images/vistoria_sistema_rodape_cliente_'+cliente_id+'.png';}
        //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        //Texto
        novaMarginTop += 5;
        texto = 'VISTORIA SISTEMA: '+'nº. '+data.success.numero_vistoria_sistema+'/'+data.success.ano_vistoria_sistema+' em '+data.success.data_prevista;
        if (traducao == 'en') {texto = await traduzirTextoGoogle(texto);}
        ...
        let perguntas = data.success.vistorias_sistemas_dados;

        //Varrer Perguntas da Vistoria Sistema
        ...
        //Parte final do Relatório (Conclusão da Vistoria Sistema)''''''''''''''''''''''''''''''''''''
        //Dados Titulo
        contadorTitulo++;
        contadorSubtitulo = 0;
        contadorPergunta = 0;
        ...
        //Parte final do Relatório (Comentários da Vistoria Sistema)''''''''''''''''''''''''''''''''''
        //Dados Titulo
        contadorTitulo++;
        contadorSubtitulo = 0;
        contadorPergunta = 0;
        ...
        let frm_vistorias_sistemas = document.getElementById('frm_vistorias_sistemas');
        ...
        frm_vistorias_sistemas.parentNode.insertBefore(link, frm_vistorias_sistemas.nextSibling);
...
document.addEventListener('DOMContentLoaded', function (event) {
    document.getElementById('vistoria_sistema_dropdown').addEventListener('click', async function(e) {
        e.preventDefault();

        const target = e.target.closest('a');

        if (target) {
            const edificacao_id = document.getElementById('edificacao_id').value;
            ...
            //Campo: edificacao_id (requerido)
            if (validacao({op:1, value:edificacao_id}) === false) {
                validacao_ok = false;
                mensagem += 'Edificação requerido.' + '<br>';
            } else {
                //Campo: edificacao_id (deve ser um número)
                if (validacao({op:4, value:edificacao_id}) === false) {
                    validacao_ok = false;
                    mensagem += 'Edificação deve ser escolhido.' + '<br>';
                }
            }
            ...
            //Incluir Vistoria Sistema''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            const vistoria_sistema_id = await executarVistoriaSistema(edificacao_id);

            await crudTable('vistorias_sistemas');
            await crudEdit(vistoria_sistema_id);
            //''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        }
    });

    //Buscar dados do Responsável escolhido
    document.getElementById('responsavel_funcionario_id').addEventListener('change', function() {
        if (document.getElementById('responsavel_funcionario_id').value == '') {
            //Inputs
            document.getElementById('responsavel_funcionario_nome').value = '';
            document.getElementById('responsavel_funcionario_email').value = '';
        } else {
            var responsavel_funcionario_id = document.getElementById('responsavel_funcionario_id').value;

            //Route: funcionarios/id
            fetch('funcionarios/'+responsavel_funcionario_id, {
                method: 'GET',
                headers: {
                    'REQUEST-ORIGIN': 'fetch',
                    'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                ...
                //Inputs
                document.getElementById('responsavel_funcionario_nome').value = funcionario.name;
                document.getElementById('responsavel_funcionario_email').value = funcionario.email;
            }).catch(error => {
                alert('Erro VistoriasSistemas:'+error);
            });
        }
    });
    ...
    document.addEventListener('click', function (e) {
        ...
        if (btnRemoverPdf) {
            ...
            removerPdf(btnRemoverPdf);
            return; // para não disparar outros handlers acidentalmente
        }

        const btnVisualizarPdf = e.target.closest('.visualizar-pdf');
        if (btnVisualizarPdf) {
            visualizarPdf(btnVisualizarPdf);
        }
    });
    //''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
});
