...
function relatorio4(op=1, relatorio_name='') {
    if (op == 1) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            //Dados
            $.get(url+'relatorios/relatorio4/'+modal_relatorio4_data+'/'+modal_relatorio4_title+'/'+modal_relatorio4_notificacao+'/'+$('#modal_relatorio4_user_id').val()+'/'+$('#modal_relatorio4_idioma').val(), function (data) {
                if (data.success) {
                    resolve(data.success);
                } else {
                    alert(data.error);
            ...
            //Gerar PDF
            gerarPDFRelatorio({x_relatorio:4, x_dados:data, x_idioma:$('#modal_relatorio4_idioma').val()});
...
function relatorio5(op=1, relatorio_name='') {
    if (op == 1) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            //Dados
            $.get(url+'relatorios/relatorio5/'+modal_relatorio5_name+'/'+modal_relatorio5_descricao+'/'+modal_relatorio5_url+'/'+$('#modal_relatorio5_user_id').val()+'/'+$('#modal_relatorio5_idioma').val(), function (data) {
                if (data.success) {
                    resolve(data.success);
                } else {
                    alert(data.error);
            ...
            //Gerar PDF
            gerarPDFRelatorio({x_relatorio:5, x_dados:data, x_idioma:$('#modal_relatorio5_idioma').val()});
    ...
    try {
        ...
        if (x_relatorio == 4) {
            ...
            registros.forEach(function (dado) {
                //Dados para preencher na linha da grade
                var v_date = dado.date;
                var v_time = dado.time;
                var v_title = dado.title;
                var v_notificacao = dado.notificacao;
                var v_user = dado.user;

                tabelaHTML += `<tr>
                            <td>${v_date ? v_date : ''}</td>
                            <td>${v_time ? v_time : ''}</td>
                            <td>${v_title ? v_title : ''}</td>
                            <td>${v_notificacao ? v_notificacao : ''}</td>
                            <td>${v_user ? v_user : ''}</td>
                        </tr>`;
        ...
        if (x_relatorio == 5) {
            ...
            registros.forEach(function (dado) {
                //Dados para preencher na linha da grade
                var v_name = dado.name;
                var v_descricao = dado.descricao;
                var v_url = dado.url;
                var v_user = dado.user;

                tabelaHTML += `<tr>
                            <td>${v_name ? v_name : ''}</td>
                            <td>${v_descricao ? v_descricao : ''}</td>
                            <td>${v_url ? v_url : ''}</td>
                            <td>${v_user ? v_user : ''}</td>
                        </tr>`;
