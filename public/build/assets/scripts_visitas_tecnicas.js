...
function atualizar_pergunta(visita_tecnica_pergunta_id) {
    ...
    //Acessar rota
    fetch(url_atual+'visitas_tecnicas/visitas_tecnicas_perguntas/atualizar_pergunta/'+visita_tecnica_pergunta_id, {
        method: 'POST',
        headers: {
            'REQUEST-ORIGIN': 'fetch',
            'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function previewPerguntas(vt_cs) {
    ...
    fetch('visitas_tecnicas/visitas_tecnicas_perguntas/perguntas_completa_sintetica/' + vt_cs, {
        method: 'GET',
