...
function vtt1_pdfEnviar(fileInput) {
    ...
    function uploadPdf(pdfFile) {
        ...
        fetch('visitas_tecnicas/pergunta/uploadPdf/' + visitaTecnicaDadoId + '/' + slotLivre, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            },
...
function vtt1_removerPdf(button) {
    ...
    fetch(`visitas_tecnicas/pergunta/removerPdf/${visita_tecnica_dado_id}/${slot}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
function vtt1_salvarDadosPergunta(visita_tecnica_dado_id) {
    ...
    let payload = {resposta, quantidade, observacao, fotografia_1, fotografia_2, fotografia_3, pdf_1, pdf_2, pdf_3};
...
function vtt1_gerarHtmlPerguntas(visitas_tecnicas_dados) {
    ...
    visitas_tecnicas_dados.forEach(dado => {
        ...
        if (dado.subtitulo) {
            ...
            html += `<div class="row col-12 bg-light ms-3 py-2">
...
async function vtt1_gerarPDF(visita_tecnica_id=0, traducao='pt', vt_cs) {
    ...
    if (data.success) {
        ...
        let existe = await fetch('visitas_tecnicas/verificar-arquivo/'+'visita_tecnica_topo_cliente_'+cliente_id+'.png', {
            method: 'GET'
        });

        let arq = await existe.json();

        if (arq.success) {
            pageTopo = 'build/assets/images/visita_tecnica_topo_cliente_'+cliente_id+'.png';
        }
        //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

        //Verificar se existe rodapé para cliente''''''''''''''''''''''''''''''''''''''''''''''''''''
        let existe2 = await fetch('visitas_tecnicas/verificar-arquivo/'+'visita_tecnica_rodape_cliente_'+cliente_id+'.png', {
            method: 'GET'
        });

        let arq2 = await existe2.json();
        ...
        if (data.success.nivel == 1) {nivel = lin1_col2;}
        if (data.success.nivel == 2) {nivel = lin2_col2;}
        if (data.success.nivel == 3) {nivel = lin3_col2;}
        if (data.success.nivel == 4) {nivel = lin4_col2;}
        if (data.success.nivel == 5) {nivel = lin5_col2;}
